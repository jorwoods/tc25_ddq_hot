...
def export_pdf(server: TSC.Server, workbook: TSC.WorkbookItem, options: TSC.PDFRequestOptions | None = None, path: str | Path | None = None) -> Path:
    """
    Export a workbook as a PDF.

    :param server: Tableau server instance
    :param workbook: WorkbookItem to export
    :param options: PDFRequestOptions for export settings
    :return: Path to the exported PDF file
    """

    request_options: TSC.PDFRequestOptions = options or TSC.PDFRequestOptions()
    pdf_path = Path(path or Path(f"{workbook.name}.pdf"))

    if options is None:
        request_options.orientation = TSC.PDFRequestOptions.Orientation.Landscape
        request_options.page_type = TSC.PDFRequestOptions.PageSize.Letter


    server.workbooks.populate_pdf(workbook, request_options)
    pdf_path.write_bytes(workbook.pdf)

    print(f"Workbook '{workbook.name}' exported as PDF to '{pdf_path}'.")
    return pdf_path
...
with server.auth.sign_in(auth):
    ...
    export_pdf(server, workbook, path=Path("workbook.pdf"))
